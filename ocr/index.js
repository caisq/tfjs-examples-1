...
import {OCRImageSynthesizer} from './synthetic_images';
...
//   const synth = new OCRImageSynthesizer(canvas, tf);
...
async function init() {
  const synth = new OCRImageSynthesizer(canvas, tf, 3);
  console.log(synth);  // DEBUG
  const {image, target} = synth.generateExample(canvas);
  console.log(image);
  console.log(target);
